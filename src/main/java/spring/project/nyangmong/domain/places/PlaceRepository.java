...
public interface PlaceRepository extends JpaRepository<Places, Integer> {
    ...
    @Query(value = "SELECT * FROM places WHERE partName = :partName ORDER BY contentSeq ASC LIMIT 0,24", nativeQuery = true)
    List<Places> searchPartName(@Param("partName") String partName, Pageable page);
